...
export default function ClientSummary({
  ...
  return (
    <div className="rounded-xl overflow-hidden" style={{ background: 'var(--ink)', border: '1px solid var(--border)' }}>
      {/* Top row: back + client info + period selector */}
      <div className="px-6 py-4 flex items-start justify-between gap-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
        <div className="flex-1 min-w-0">
          {onBackToLibrary && (
            <button onClick={onBackToLibrary} className="text-xs mb-2 flex items-center gap-1.5 transition-all hover:opacity-80" style={{ color: '#9ca3af' }}>
              ← Back to Library
            </button>
          )}
          <p className="text-xs uppercase tracking-widest mb-1" style={{ color: 'var(--accent)' }}>A/P Aging Detail</p>
          <h1 className="text-2xl font-bold leading-tight" style={{ color: 'var(--paper)', fontFamily: 'Playfair Display, serif' }}>
            {clientName}
          </h1>
          {asOfDate && (
